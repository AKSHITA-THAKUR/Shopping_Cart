    ...
    <div className=" max-w-md my-20 mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl">
      ...
      <div className="md:flex">
        <div className="md:flex-shrink-0">
          <img src={product.image} alt={product.title} className="h-48 w-full object-cover md:w-48" />
        </div>
        <div className="p-8">
          ...
          <div className="mt-4 flex items-center">
            ...
            <button onClick={() => addToCart(product)} className="ml-auto bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                Add to Cart
              </button>
              <div className="ml-2">
                <button onClick={decCount} className="text-lg text-gray-500" disabled={cartCount === 0}>-</button>
                <span className="mx-2 text-lg">{cartCount}</span>
                <button onClick={increaseCount} className="text-lg text-gray-500" disabled={cartCount === 5}>+</button>
              </div>
          </div>
        </div>
      </div>
